...
struct prime_number_hash_policy {
    ...
    size_t index_for_hash(size_t hash, size_t) const { return current_mod_function(hash); }
...
struct fibonacci_hash_policy {
    size_t index_for_hash(size_t hash, size_t) const { return (11400714819323198485ull * hash) >> shift; }
...
public:
    ...
    std::pair<typename Table::iterator, bool> try_emplace(const key_type& key, Args&&... args) {
        ...
        return this->emplace(key, V(std::forward<Args>(args)...));
    }
    template <typename... Args> std::pair<typename Table::iterator, bool> try_emplace(key_type&& key, Args&&... args) {
        auto found = this->find(key);
        if (found != this->end()) {
            return {found, false};
        }
        return this->emplace(std::move(key), V(std::forward<Args>(args)...));
    }
